...
describe("FileSystem Tools (Pro Test Suite)", () => {
    ...
    describe("validateAndResolvePath", () => {
        ...
        it("路径清洗：应该自动剥离大模型传入的绝对路径根斜杠", () => {
            // 测试单斜杠
            const result1 = validateAndResolvePath(TEST_ROOT, "/src/components/Button.tsx");
            expect(result1).toBe(path.join(TEST_ROOT, "src/components/Button.tsx"));

            // 测试多斜杠 (大模型有时候会发癫输出 //docs)
            const result2 = validateAndResolvePath(TEST_ROOT, "//docs/README.md");
            expect(result2).toBe(path.join(TEST_ROOT, "docs/README.md"));

            // 测试根目录本身
            const result3 = validateAndResolvePath(TEST_ROOT, "/");
            expect(result3).toBe(TEST_ROOT);
        });

        it("边界测试：清洗斜杠后不影响合法的子目录同名检验", () => {
            // 确保 /src 不会被错误解析到操作系统的 /src，而是映射到当前项目的根目录下
            const result = validateAndResolvePath(TEST_ROOT, "/Users/xxx/projects");
            // 因为开头有 '/'，它会被清洗成 "Users/xxx/projects"，作为相对路径拼在 TEST_ROOT 后面
            expect(result).toBe(path.join(TEST_ROOT, "Users/xxx/projects"));
        });
    });
